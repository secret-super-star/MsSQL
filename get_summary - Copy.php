...
while($obj = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)){
    ...
    $tsql = "select * from AccountsBalance where FORMAT(TimeStamp,'yyyy') = FORMAT( GETDATE( ) ,'yyyy' ) and ClientUsername = '".$client_name."'";
...
while($obj = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)){
    $client_name = $obj['Bet365User'];
    $tsql = "select * from AccountsBalance where TimeStamp >= (CURRENT_TIMESTAMP-30) and TimeStamp <= (CURRENT_TIMESTAMP-1) and ClientUsername = '".$client_name."' order by TimeStamp ASC";
    $stmt1 = sqlsrv_query( $conn, $tsql);
    while($obj = sqlsrv_fetch_array($stmt1, SQLSRV_FETCH_ASSOC)){
        $timestamp = strtotime($obj['TimeStamp']);
        $php_date = getdate($timestamp);
...
while($obj = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)){
    $client_name = $obj['Bet365User'];
    $tsql = "select * from AccountsBalance where TimeStamp >= (CURRENT_TIMESTAMP-6) and TimeStamp <= (CURRENT_TIMESTAMP-1) and ClientUsername = '".$client_name."' order by TimeStamp ASC";
    $stmt1 = sqlsrv_query( $conn, $tsql);
    while($obj = sqlsrv_fetch_array($stmt1, SQLSRV_FETCH_ASSOC)){
        $timestamp = strtotime($obj['TimeStamp']);
        $php_date = getdate($timestamp);
...
while($obj = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)){
    $client_name = $obj['Bet365User'];
    $tsql = "select * from AccountsBalance where TimeStamp >= DATEADD(hh, -24, GETDATE()) and ClientUsername = '".$client_name."' order by TimeStamp ASC";
